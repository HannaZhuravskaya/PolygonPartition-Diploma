...
#pragma region  Mesh-string Convertation
	...
	Mesh* Mesh::convertFromString(std::string stringData) {
		try {
			...
			std::getline(ss, to);
			data = Helper::splitStringByRegex(to, "V: |, E: |, F: ");
			...
			for (int i = 0; i < numOfV; ++i) {
				std::getline(ss, to);
				data = Helper::splitStringByRegex(to, "[^0-9.]+");
				int cnt = 1;
...
#pragma region Methods used in the algorithm
	...
	std::pair<Point*, Point*> Mesh::findPointsV(double x1, double y1, double x2, double y2, double x3, double y3, double x4, double y4, double S) {
		...
		//3-rightTop
		//4-rightBottom
		auto coeff1 = Helper::findLineCoefficients(x4, y4, x1, y1), coeff2 = Helper::findLineCoefficients(x2, y2, x3, y3);
		...
		auto roots1 = Helper::findRootsOfEquation(a, b, c_neg, EPS);
		auto roots2 = Helper::findRootsOfEquation(a, b, c_pos, EPS);

		std::vector<double>* roots = new std::vector<double>();
		Helper::addValidRootsToList(roots1, roots);
		Helper::addValidRootsToList(roots2, roots);
...
#pragma region Methods used in the algorithm
	...
	std::vector<Vertex*>  Mesh::splitByVerticalLineInFace(Face* f, double coord) {
		...
		do {
			...
			if ((Helper::isLess(startV->pos->x, coord) && Helper::isGreater(endV->pos->x, coord))
				|| (Helper::isGreater(startV->pos->x, coord) && Helper::isLess(endV->pos->x, coord))) {

				if (Helper::isEqual(startV->pos->y, endV->pos->y)) {
			...
			if (Helper::isEqual(startV->pos->x, coord) && vertexesToJoinMap.find(startV) == vertexesToJoinMap.end())
				vertexesToJoinMap.insert(startV);
			if (Helper::isEqual(endV->pos->x, coord) && vertexesToJoinMap.find(endV) == vertexesToJoinMap.end())
	...
	std::vector<Vertex*>  Mesh::splitByHorizontalLineInFace(Face* f, double coord) {
		std::set<Vertex*> vertexesToJoinMap;
		std::vector<Vertex*> vertexesToJoin;
		std::map<Edge*, Point*> vertexesToAdd;
		std::vector<Vertex*> faceVertexes;
		...
		do {
			...
			if ((Helper::isLess(startV->pos->y, coord) && Helper::isGreater(endV->pos->y, coord))
				|| (Helper::isGreater(startV->pos->y, coord) && Helper::isLess(endV->pos->y, coord))) {
				if (Helper::isEqual(startV->pos->x, endV->pos->x)) {
			...
			if (Helper::isEqual(startV->pos->y, coord) && vertexesToJoinMap.find(startV) == vertexesToJoinMap.end())
				vertexesToJoinMap.insert(startV);
			if (Helper::isEqual(endV->pos->y, coord) && vertexesToJoinMap.find(endV) == vertexesToJoinMap.end())
				vertexesToJoinMap.insert(endV);
	...
	std::vector<Vertex*> Mesh::splitByListOfVertexes(std::map<Edge*, Point*> toAddAndJoin, std::vector<Vertex*> toJoin, std::vector<Vertex*> faceVertexes)
	{
		...
		sort(toJoin.begin(), toJoin.end(), [](const Vertex* p1, const Vertex* p2)
			{ return Helper::isGreater(p1->pos->y, p2->pos->y) || Helper::isLess(p1->pos->x, p2->pos->x); });
	...
	int Mesh::isPointInPolygon(Point* P, std::vector<Vertex*> faceVertexes)
	{
		...
		for (int i = 0; i < faceVertexes.size(); i++) {   // edge from V[i] to  V[i+1]
			if (Helper::isLess(faceVertexes[i]->pos->y, P->y) || Helper::isEqual(faceVertexes[i]->pos->y, P->y)) {          // start y <= P.y
				if (Helper::isGreater(faceVertexes[(i + 1) % faceVertexes.size()]->pos->y, P->y))      // an upward crossing
				{
					double pos = Helper::isLeft(faceVertexes[i]->pos, faceVertexes[(i + 1) % faceVertexes.size()]->pos, P);
					if (Helper::isEqual(pos, 0))
						return 0;
					if (Helper::isGreater(pos, 0))  // P left of  edge
						++wn;            // have  a valid up intersect
				}
			}
			else {                        // start y > P.y (no test needed)
				if (Helper::isLess(faceVertexes[(i + 1) % faceVertexes.size()]->pos->y, P->y) || Helper::isEqual(faceVertexes[(i + 1) % faceVertexes.size()]->pos->y, P->y))     // a downward crossing
				{
					double pos = Helper::isLeft(faceVertexes[i]->pos, faceVertexes[(i + 1) % faceVertexes.size()]->pos, P);
					if (Helper::isEqual(pos, 0))
						return 0;
					if (Helper::isLess(pos, 0))  // P right of  edge
	...
	std::vector<int> Mesh::findConcavePoints()
	{
		...
		for (int i = 0; i < size; ++i) {
			...
			if ((Helper::isEqual(prev->x, cur->x) && Helper::isEqual(cur->x, next->x))
				|| (Helper::isEqual(prev->y, cur->y) && Helper::isEqual(cur->y, next->y)))
				continue;

			auto nearPrev = Helper::findPointOnSegmentByDistance(cur, prev, 0.1, EPS);
			auto nearNext = Helper::findPointOnSegmentByDistance(cur, next, 0.1, EPS);
	...
	void Mesh::splitToConvexPolygonsByPermutation(std::vector<int> permutation, int n, std::set<int> usedVertexes, bool isVertical, Mesh* mesh, std::vector<Mesh*>* allMeshes)
	{
		if (usedVertexes.find(permutation[n]) == usedVertexes.end()) {
			...
			std::vector<Vertex*> vertexes;
			if (isVertical) {
				vertexes = mesh->splitByVerticalLineInFace(f, v->pos->x);
			}
			else {
				vertexes = mesh->splitByHorizontalLineInFace(f, v->pos->y);
			}
	...
	double Mesh::getPartAreaToSplitFace(Face* face, double minS, double maxS) {
		if (face->area > minS&& face->area < maxS)
			return face->area;
